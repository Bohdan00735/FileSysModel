...
class Device(val sizeOfBlock: Int) {
    ...
    private val fileSystemSize = 20000
    ...
    fun getChartArray(start: Int): List<Byte> {
        if (start+8 < filesystemArray.size) throw SystemError("Block out of bound")
    ...
    fun writeBlock(blockIndex: Int, block: ByteArray) {
        if (blockIndex+8 < filesystemArray.size) throw SystemError("Block out of bound")
